...
public class FileSystemConverter {
     ...
     * @throws IOException
     */
    public static void toPhysicalFileSystem(POIFSFileSystem poifs,
    ...
    public static POIFSFileSystem toOLEFileSystem(String importDir)
            throws IOException {
    ...
    private static void readFromFileSystem(DirectoryEntry dirEntry,
            String importDir) throws IOException {
        ...
        for (int i = 0; i < entryCount; i++) {
            ...
            if (file.isDirectory()) {
                DirectoryEntry childDirEntry = dirEntry
                        .createDirectory(childEntryName);
    ...
    private static byte parseByte(String rawString, int offset) {
        return (byte) Integer.parseInt(rawString.substring(offset, offset + 2),
                16);
    }

    /**
     * recursive하게 돌면서 File System으로 옮겨 담는다
     * 
     * @throws IOException
     */
    private static void writeToFileSystem(DirectoryEntry dirEntry,
            String outputPath) throws IOException {
        // Depth First 방식
        ...
        FileOutputStream metaInfoOS = new FileOutputStream(outputPath + "/"
